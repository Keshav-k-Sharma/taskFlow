...
export default function MemberCard({ member, isAdmin, onUpdate }) {
    const [editing, setEditing] = useState(false);
    const [position, setPosition] = useState(member.position);

    const handleUpdate = async () => {
        await api.patch(`/api/members/${member._id}`, { position });
        setEditing(false);
        onUpdate();
    };
    ...
    return (
        <div style={{ padding: "0.75rem", backgroundColor: "#0a0a0a", border: "1px solid #2a2a2a", borderRadius: "6px", marginBottom: "0.5rem" }}
            ...
            {editing ? (
                ...
                <span style={{ marginTop: "0.4rem", display: "inline-block", backgroundColor: "#2a2a2a", color: "#f5e642", padding: "0.15rem 0.5rem", borderRadius: "4px", fontSize: "0.75rem" }}>{member.position}</span>
            )}
        </div>
    );
}